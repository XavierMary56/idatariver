...
type InstrumentRepo struct{ db *pgxpool.Pool }

func NewInstrumentRepo(db *pgxpool.Pool) *InstrumentRepo { return &InstrumentRepo{db: db} }
...
func (r *InstrumentRepo) InstrumentID(ctx context.Context, symbol string) (int64, error) {
	const q = `select id from instrument where symbol=$1 and active=true limit 1;`
	var id int64
	if err := r.db.QueryRow(ctx, q, symbol).Scan(&id); err != nil {
		return 0, err
	}
	return id, nil
}

func (r *InstrumentRepo) ListSymbols(ctx context.Context, onlyActive bool, types []string, tenantID int64) ([]SymbolRow, error) {
	const q = `
with base as (
...
order by e.type asc, e.symbol asc;`
	...
	rows, err := r.db.Query(ctx, q, onlyActive, types, tenantID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
